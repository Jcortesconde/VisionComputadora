...
intensity_cols = dim(intensity)(1);
intensity_rows = dim(intensity)(2);
intensity = [ zeroes(intensity_cols, 1)  intensity  zeroes(intensity_cols, 1)];
intensity = [ zeroes(intensity_rows + 1, 1)  intensity  zeroes(intensity_rows + 1, 1)];
grad = zeros(dim(intensity));
intensity_cols++;
intensity_rows++;
%%% Gradientes cero;
